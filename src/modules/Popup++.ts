...
const configSchema = {
  ...
  label: "<span tooltip='Stacking popups,click on popups to make it disappear'>Popup++</span>",
  default: true,
} as const;
export default class PopupPlusPlus extends HHModule {
  ...
  run() {
    ...
    shared.general.objectivePopup.show = function(t : any) {
        ...
        if(!t.objective_points){
            ...
            if (Object.keys(this.pointsBox).length){
                for (let t in n){
                    this.pointsBox[t] ? this.pointsBox[t].name === n[t].name && (this.pointsBox[t].points_gained += n[t].points_gained) : this.pointsBox[t] = n[t];
                }
            }
            else{
    ...
    function customPopup(_objectivePopupthis: any,objective_points : any){
        ...
        const $popup = $(`<div class="popup_wrapper"><div id="objective_popup" class="popup"><div class="noti_box"><div class="points">${
        Object.keys(lastPoints).map((n =>{
            const currObjective = lastPoints[n]
            const animateClass = "row animate"
            return `<div class="${animateClass}" style="transition: all 20ms;"><div class="contest_name">${currObjective.title}:</div><div class="contest_points"><div class="points_name" style="animation:none;">${currObjective.name}: </div><div class="points_num" style="animation:none;"><div class="points_i" style="animation:none;">+${number_format_lang(currObjective.points_gained)}</div></div></div></div>`
